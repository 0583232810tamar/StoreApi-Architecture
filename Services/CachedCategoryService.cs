...
// Decorator over ICategoryService that adds Redis caching.
// Categories change infrequently, so a longer TTL (300s default) is used.
// When to use Redis here:
//   - GET /categories        → cached (static-ish data, read very often for dropdowns/navigation)
//   - GET /categories/{id}   → cached per category
//   - POST/PUT/DELETE        → invalidate all category keys + also invalidate product caches
//                              because ProductResponseDto embeds CategoryName
public class CachedCategoryService : ICategoryService
{
    ...
    public async Task<CategoryResponseDto> CreateCategoryAsync(CategoryCreateDto createDto)
    {
        ...
        await InvalidateCategoryAndProductCachesAsync();
        return result;
    }
    ...
    public async Task<CategoryResponseDto?> UpdateCategoryAsync(int id, CategoryUpdateDto updateDto)
    {
        ...
        if (result != null) await InvalidateCategoryAndProductCachesAsync();
        return result;
    }
    ...
    public async Task<bool> DeleteCategoryAsync(int id)
    {
        ...
        if (result) await InvalidateCategoryAndProductCachesAsync();
        return result;
    }

    // Category mutations also bust product caches because ProductResponseDto embeds CategoryName.
    private async Task InvalidateCategoryAndProductCachesAsync()
    {
